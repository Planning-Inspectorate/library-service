...
namespace Drupal\pins\Plugin\search_api\processor;
 ...
 *
 * @SearchApiProcessor(
 *   id = "custom_file_attachments",
 *   label = @Translation("Custom File attachments"),
 *   description = @Translation("Adds the custom file attachments content to the indexed data."),
 *   stages = {
 *     "add_properties" = 0,
 *   }
 * )
 */
class CustomFilesExtractor extends ProcessorPluginBase implements PluginFormInterface {
   ...
   *
   * @var \Drupal\pins\Services\IndexingLogService $indexingLogService
   */
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    return new static(
        ...
        $container->get('pins.indexing_log')
    );
  }
        ...
        foreach ($this->fieldHelper->filterForPropertyPath($item->getFields(), NULL, $property_path) as $field) {
          ...
          if (!empty($files)) {
            ...
            foreach ($files as $file) {
              // Update the file entity with the new URI which are symlinked from blob storage
  ...
  public function extractOrGetFromCache(EntityInterface $entity, File $file, TextExtractorPluginInterface $extractor_plugin) {
    ...
    if ($cache = $this->keyValue->get($collection)->get($key)) {
      ...
      try {
        ...
        $fallback_collection = $this->keyValue->get(CustomFilesExtractor::FALLBACK_QUEUE_KV);
                  ...
                  try {
                    if (!empty($this->entityTypeManager->getStorage('media_type')->load($bundle_name))) {
                      ...
                      if (isset($bundle_configuration['source_configuration']['source_field'])) {
                        ...
                        if ($field_config = $this->entityTypeManager->getStorage('field_storage_config')->load(sprintf('media.%s', $source_field))) {
