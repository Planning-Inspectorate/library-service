...
resource "azurerm_subnet" "mysql" {
  ...
  name                              = "${local.org}-snet-${local.service_name}-mysql-${var.environment}"
  resource_group_name               = azurerm_resource_group.primary.name
  virtual_network_name              = azurerm_virtual_network.main.name
  address_prefixes                  = [var.vnet_config.mysql_subnet_address_space]
  private_endpoint_network_policies = "Enabled"
...
resource "azurerm_virtual_network_peering" "library_to_tooling" {
...
resource "azurerm_virtual_network_peering" "tooling_to_library" {
...
resource "azurerm_private_dns_zone_virtual_network_link" "managed_redis" {
  name                  = "${local.org}-vnetlink-managed-redis-${local.resource_suffix}"
  resource_group_name   = var.tooling_config.network_rg
  private_dns_zone_name = data.azurerm_private_dns_zone.managed_redis.name
  virtual_network_id    = azurerm_virtual_network.main.id
